...
const Art = (props) => {
    ...
    const prevPage = currentPage - 1 === 1 ? '/category/art' : `/category/art/${currentPage - 1}`
    const nextPage = `/category/art/${currentPage + 1}`

    return (
        <Layout>
        <SEO title='Blog' keywords={['art']} />
        <Nav />
        ...
        <div className='feed'>
            ...
            <p className='card__category'>{category.category}</p> // here category
            ))}
...
export const pageQuery = graphql` 
 query ArtQuery ($skip: Int!, $limit: Int!) {
   allContentfulBlog(
       sort: { fields: [createdAt], order: DESC }
       filter: {
       node_locale: {eq: "en-US",}
       category: {elemMatch: { "Art"}}
    }
       skip: $skip
       limit: $limit
     ) {
